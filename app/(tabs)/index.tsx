...
function FileThumbnail({
  ...
  if (file.kind === "image" || previewUri) {
    return (
      <Image
        source={{ uri: previewUri ?? file.uri }}
...
export default function LibraryScreen() {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (imported.length > 0) {
        const nextFiles = [...imported, ...files].sort(
      ...
      if (skipped.length > 0) {
        ...
        Alert.alert(
          "没有可导入的图片",
          "请选择 ARW、CR2、CR3、NEF、RW2、PNG、JPG 或 JPEG 文件。",
        );
      }
    } catch {
  ...
  const edgeBackGesture = useMemo(
    () =>
      Gesture.Pan()
        ...
        .onEnd((event) => {
          const shouldDismiss =
            event.translationX > 86 || event.velocityX > 900;
          if (shouldDismiss) {
            detailOffsetX.value = withTiming(
              420,
              { duration: 180 },
              (finished) => {
  ...
  const confirmRename = useCallback(async () => {
    ...
    try {
      ...
      const nextFiles = files.map((file) =>
        file.id === renamed.id ? renamed : file,
      );
      setFiles(nextFiles);
      ...
      setRenameNotice(
        outcome.sourceRenamed
          ? `已同步改名为 ${renamed.fileName}`
          : `已改本地副本为 ${renamed.fileName}`,
      );
      feedback("success");
    } catch (error) {
      const message =
  ...
  if (selectedFile) {
    const rawPreviewUri =
      rawPreviewState.fileId === selectedFile.id ? rawPreviewState.uri : null;
    const isRawPreviewLoading =
      selectedFile.kind === "raw" && rawPreviewState.status === "loading";
    const canRenderImage =
      (selectedFile.kind === "image" && !imageFailed) || Boolean(rawPreviewUri);
    return (
              ...
              {canRenderImage ? (
                ...
                <View style={styles.rawPreview}>
                  ...
                  <Text style={styles.rawPreviewText}>
                    首次预览会在设备本地生成 PNG 缓存，完成后将直接显示图像。
                  </Text>
                </View>
              ) : (
                <View style={styles.rawPreview}>
                  <View style={styles.rawPreviewIcon}>
                    <MaterialIcons
                      name={
                        selectedFile.kind === "raw" ? "camera" : "broken-image"
                      }
                      size={42}
                      color="#D7983D"
                    />
                  </View>
                  <Text style={styles.rawPreviewTitle}>
                    {selectedFile.kind === "raw"
                      ? `${selectedFile.brand} RAW`
                      : "无法渲染预览"}
                  </Text>
                  <Text style={styles.rawPreviewText}>
                    {selectedFile.kind === "raw"
                  ...
                  {selectedFile.kind === "raw" && (
                    <Pressable
                      onPress={() => setRawPreviewRetry((value) => value + 1)}
                      style={({ pressed }) => [
                        styles.retryButton,
                        pressed && styles.pressed,
                      ]}
              ...
              <View style={styles.fileTitleRow}>
                <View style={styles.titleTextBox}>
                  ...
                  <Text style={styles.fileMeta}>
                    {selectedFile.brand} · {formatBytes(selectedFile.size)}
                  </Text>
                </View>
              ...
              {renameNotice && (
                <View style={styles.renameNotice}>
                  <MaterialIcons
                    ...
                    color="#69C99A"
                  />
                  <Text style={styles.renameNoticeText} numberOfLines={1}>
                    {renameNotice}
                  </Text>
                </View>
              )}
    ...
    <ScreenContainer className="flex-1" containerClassName="bg-background">
        ...
        <View>
          ...
          <View style={styles.libraryStatusRow}>
            ...
            <Text style={styles.libraryStatusText}>
              已保存 {files.length} 个本地副本，其中 {rawFileCount} 个 RAW
            </Text>
          </View>
        </View>
        <Pressable
      ...
      {isLoading ? (
        <View style={styles.centeredState}>
          ...
          <Text style={styles.loadingText}>正在读取本地图库</Text>
        </View>
      ) : (
        <FlatList
          data={filteredFiles}
          ...
          contentContainerStyle={
            filteredFiles.length === 0 ? styles.emptyList : styles.listContent
          }
          renderItem={({ item }) => (
            <Pressable
            ...
            >
              <FileThumbnail
                file={item}
                previewUri={rawPreviewCache.current[item.id]}
              />
              <View style={styles.fileCardContent}>
                ...
                <Text style={styles.fileCardMeta}>
                  {item.brand} · {formatBytes(item.size)}
                </Text>
                <View style={styles.fileCardFooter}>
                  <View style={styles.smallBadge}>
                    <Text style={styles.smallBadgeText}>{fileBadge(item)}</Text>
                  </View>
                  <Text
                    style={[
                      styles.localCopyText,
                      item.renameSyncStatus === "copy_only" &&
                        styles.localCopyWarning,
                    ]}
                  >
                    {item.renameSyncStatus === "original_and_copy"
                      ? "已同步改名"
                      : item.renameSyncStatus === "copy_only"
                        ? "已改本地副本"
                        : "本地副本"}
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <View style={styles.emptyIcon}>
                <MaterialIcons name="photo-library" size={36} color="#D7983D" />
              </View>
              <Text style={styles.emptyTitle}>还没有导入的文件</Text>
              <Text style={styles.emptyText}>
                从设备文件中选择相机 RAW、PNG 或 JPG
                图片。导入后会在应用内保存一份本地副本。
              </Text>
            </View>
          }
...
function RenameModal({
  ...
  const isReady =
    Boolean(cleanName) && nextFileName !== file.fileName && !isSaving;
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.bottomSheet}>
          ...
          <Text style={styles.modalDescription}>
            仅编辑名称部分，扩展名会保持为 .{file.extension.toUpperCase()}。
          </Text>
          <View style={styles.renameInputRow}>
            ...
            {value.length > 0 && (
              <Pressable
                onPress={onClear}
                style={({ pressed }) => [
                  styles.clearRenameButton,
                  pressed && styles.pressed,
                ]}
                accessibilityLabel="清空文件名"
              >
          ...
          <View style={styles.renamePreviewRow}>
            ...
            <Text style={styles.renamePreviewName} numberOfLines={1}>
              {nextFileName || "请输入有效名称"}
            </Text>
          </View>
          <Text style={styles.renameHelperText}>
            会优先改名原始文件；若来源不允许写入，至少会更新应用内的本地副本。
          </Text>
          <View style={styles.modalActions}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                ...
                (pressed || isSaving) && isReady && styles.primaryButtonPressed,
              ]}
            >
              {isSaving ? (
                <ActivityIndicator color="#11161C" />
              ) : (
                <Text style={styles.confirmButtonText}>确认重命名</Text>
              )}
            </Pressable>
          </View>
        </View>
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.supportCard}>
          <View style={styles.supportTitleRow}>
            <View>
              <Text style={styles.modalTitle}>支持格式</Text>
              <Text style={styles.modalDescription}>
                导入时自动识别相机品牌与文件类别。
              </Text>
            </View>
            ...
            >
              <MaterialIcons name="close" size={21} color="#F4F1EA" />
            </Pressable>
          </View>
          <View style={styles.supportList}>
            {supportedFormats.map(([brand, format]) => (
              <View key={brand} style={styles.supportRow}>
                <Text style={styles.supportBrand}>{brand}</Text>
                <Text style={styles.supportFormat}>{format}</Text>
              </View>
            ))}
          </View>
          <View style={styles.noticeBox}>
            ...
            <Text style={styles.noticeText}>
              PNG、JPG 与 JPEG 可直接预览。RAW 文件会在 Android
              原生构建中解码并生成设备本地 PNG 缓存；Expo Go
              不包含该原生解码器。
            </Text>
          </View>
        </View>
