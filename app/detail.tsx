...
import { copyExifInfo, getExifDisplayRows, readExifInfo, shareExifInfo, type ExifInfo } from "@/lib/exif-info";
import { CROP_ASPECT_RATIOS, createCroppedLibraryCopy, type CropAspectRatio } from "@/lib/photo-crop";
...
export default function DetailScreen() {
  ...
  const copyExif = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert("复制未完成", error instanceof Error ? error.message : "无法复制 EXIF 信息。请重试。");
    }
  }, [exifState.info, file]);
  ...
  const shareExif = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert("分享未完成", error instanceof Error ? error.message : "无法打开系统分享面板。请改用复制 EXIF 信息。");
    }
  }, [exifState.info, file]);
  ...
  const openCrop = useCallback(() => {
    if (!file) return;
    if (file.kind !== "image") {
      Alert.alert("暂不支持 RAW 裁切", "为保留原始 RAW 数据，当前按比例裁切仅支持 PNG、JPG 和 JPEG。本文件仍可正常预览和导出。");
      return;
    }
    setCropRatio("1:1");
  ...
  const confirmCrop = useCallback(async () => {
    ...
    setIsCropping(true);
    try {
      const croppedFile = await createCroppedLibraryCopy(file, cropRatio);
      setIsCropVisible(false);
      setNotice(`已创建居中 ${cropRatio} 裁切副本`);
      hapticSuccess();
      router.push({ pathname: "/detail", params: { id: croppedFile.id } });
    } catch (error) {
      Alert.alert("裁切未完成", error instanceof Error ? error.message : "无法保存裁切副本。请重试。");
    } finally {
      setIsCropping(false);
    }
  }, [cropRatio, file, router]);
      ...
      <Animated.View style={[styles.flex, pageStyle]}>
        <ScreenContainer className="flex-1" containerClassName="bg-background">
          ...
          <View style={styles.sheet}>
            ...
            <Pressable onPress={openCrop} style={({ pressed }) => [styles.cropButton, pressed && styles.renameButtonPressed]}><MaterialIcons name="crop" size={18} color="#D7C6F7" /><Text style={styles.cropButtonText}>按比例裁切</Text><Text style={styles.cropHint}>{isImage ? "保存新副本" : "仅图片"}</Text><MaterialIcons name="chevron-right" size={20} color="#B7A2EA" /></Pressable>
          </View>
...
const styles = StyleSheet.create({
  ...
  cropButton: { minHeight: 46, marginTop: 9, borderRadius: 14, borderWidth: 1, borderColor: "#554475", backgroundColor: "#241E31", paddingHorizontal: 14, flexDirection: "row", alignItems: "center", gap: 8 },
  cropButtonText: { flex: 1, color: "#E2D7F7", fontSize: 13, fontWeight: "800" },
  ...
  exifQuickButton: { height: 42, flex: 1, borderRadius: 12, borderWidth: 1, borderColor: "#315069", backgroundColor: "#172633", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  ...
  cropSheet: { backgroundColor: "#1B242D", borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingHorizontal: 22, paddingBottom: 28, paddingTop: 9, borderTopWidth: 1, borderColor: "#594A76" },
  cropIcon: { width: 38, height: 38, borderRadius: 19, alignItems: "center", justifyContent: "center", backgroundColor: "#241E31", borderWidth: 1, borderColor: "#554475" },
  ratioGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 20 },
  ratioButton: { width: "30%", minHeight: 52, borderRadius: 14, borderWidth: 1, borderColor: "#4D5C67", backgroundColor: "#11161C", alignItems: "center", justifyContent: "center" },
  ...
  cropDescription: { marginTop: 15, color: "#B9A8D7", fontSize: 12, lineHeight: 18 },
  ...
  cropConfirmButton: { height: 48, flex: 1.6, borderRadius: 14, backgroundColor: "#B7A2EA", alignItems: "center", justifyContent: "center" },
