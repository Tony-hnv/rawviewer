...
当前开发版本为 **1.2.0**，Android `versionCode` 为 **19**。本轮加入自由比例手动裁切、品牌 Logo 显示/隐藏、60%–160% 尺寸调节、水平/垂直位置微调，并将边框编辑器改为可滚动的分组操作面板。该版本已通过单元测试、TypeScript、ESLint、Prettier、Expo 配置解析和 Android prebuild 注入检查；尚未生成新的公开 Release，也尚未完成真实 Android 设备端到端验收。
...
| 裁切与边框 | 当前仅对 PNG、JPG、JPEG 生成新副本；裁切支持固定或自由比例，RAW 保持只读。 |
| 真机验收   | 1.2.0 的自由裁切、Logo 调节和 UI 改动尚未完成真实 Android 设备端到端验证。 |
