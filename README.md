...
<p align="center">
  ...
  导入、预览、管理、旋转、裁切、AI 修图与安全导出，不改动设备原始文件。
</p>
...
| 常见图片      | `.png`、`.jpg`、`.jpeg` | 预览、重命名、旋转、裁切、AI 修图、照片边框与导出 |
...
| 手动裁切   | `1:1`、`4:3`、`3:4`、`16:9`、`9:16` 或自由比例；拖动位置并缩放选区     | 仅对 PNG、JPG、JPEG 生成新副本         |
| 图片旋转   | 左转 / 右转 90°，生成独立旋转副本                                      | 直立解码后旋转；原图保持不变           |
| AI 修图    | 自定义 OpenAI-compatible API；自然提亮、降噪、去杂物、胶片色调等提示词 | 仅上传普通图片；结果生成新副本         |
...
| 品牌标识  | 品牌 Logo 与品牌名称 | 支持显示/隐藏、大小和位置调节     |

品牌标识模板使用可辨识的单色透明品牌 Logo，而不是相机 / 手机类别图形或首字母徽章。Logo 可单独隐藏，也可按 60%–160% 调整大小，并在底部信息栏内进行水平、垂直微调。目前覆盖 **Sony、Canon、Nikon、Fujifilm、Leica、Hasselblad、Panasonic、Apple、Samsung、Google、Huawei、Xiaomi、OPPO、vivo**。Logo 仅用于用户选择品牌信息时的识别展示，不表示 RAW View 与相关商标权利人存在关联、授权或背书。资源来源与维护说明见 [`assets/brand-logos/SOURCES.md`](assets/brand-logos/SOURCES.md)。
...
选择固定比例或“自由”模式后，拖动裁切框调整位置，拖动右下角手柄或使用双指手势调整选区大小。固定比例会保持宽高比；自由模式允许宽度和高度独立变化。应用会将显示坐标换算到经过 EXIF 方向归一化的原图像素坐标，再写入新的应用私有副本。原图与原始导入文件保持不变。

### 图片旋转

在普通图片详情页使用“左转”或“右转”按钮，每次旋转 90°。应用会先按 EXIF 方向解码，再将旋转结果保存到 `raw-view-library/` 的新副本；原始图片和已有副本不会被覆盖。RAW 文件保持只读。

### AI 修图

在普通图片详情页打开“AI 修图”，首次使用先进入 AI 服务设置，填写 OpenAI-compatible API 地址、API Key 和图像模型。当前请求使用 `POST /v1/images/edits`，兼容返回图片 URL 或 Base64 图片数据的服务。编辑面板提供自然提亮、降噪、去除背景杂物和胶片色调等提示词，也支持自行描述修改内容。

API 地址和模型保存在本地配置中，API Key 在 Android 使用 SecureStore / Keystore 保存；图片会直接传输到用户配置的服务商。使用公网服务时应优先使用 HTTPS，局域网自建服务可按实际网络使用 HTTP。AI 返回结果会先写入临时缓存，再复制到应用私有图库并校验入库，原图不会被覆盖。
...
RAW View 使用 Expo SDK 54 与 React Native 构建，采用 Expo Router 管理页面，AsyncStorage 保存本地图库索引，Android 原生 Kotlin 模块负责 RAW 解码、EXIF / 厂商元数据、EXIF 方向归一化裁切和照片边框合成。品牌 Logo 以离线透明 PNG 资源打包，在 React Native 编辑器和 Android 原生导出模块中共用稳定的品牌映射。编辑器提供显示开关、60%–160% 尺寸步进和五向位置调节，原生导出会将参数限制在安全的信息栏范围内。
...
| 本地业务   | `lib/photo-library.ts`、`lib/photo-crop.ts`、`lib/photo-rotate.ts`、`lib/photo-frame.ts`、`lib/ai-image.ts` | 副本、图库索引、旋转、边框与 AI 保存                  |
...
| 测试       | `tests/*.test.ts`                                                                                           | RAW 文件模型、EXIF、裁切、旋转、AI 配置和边框回归测试 |
...
当前开发版本为 **1.3.0**，Android `versionCode` 为 **20**。本轮新增图片左转 / 右转 90°安全副本、OpenAI-compatible 图像编辑服务配置、SecureStore / Keystore API Key 保存、常用 AI 修图提示词、AI 结果 URL / Base64 落盘校验，并保留 1.2.0 的自由比例裁切和 Logo 调节能力。该版本已通过单元测试、TypeScript、ESLint、Prettier、Expo 配置解析和 Android prebuild 注入检查；尚未生成新的公开 Release，也尚未完成真实 Android 设备端到端验收。
...
| 裁切、旋转与边框 | 当前仅对 PNG、JPG、JPEG 生成新副本；裁切支持固定或自由比例，旋转支持 90°倍数，RAW 保持只读。        |
| AI 修图          | 需要用户提供兼容 `/v1/images/edits` 的服务、模型和 API Key；RAW 当前不能直接上传。                  |
| 真机验收         | 1.3.0 的旋转、AI 请求、结果落盘、自由裁切、Logo 调节和 UI 改动尚未完成真实 Android 设备端到端验证。 |
